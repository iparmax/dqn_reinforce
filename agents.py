    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        :param max_timestep (int): maximum timesteps that the training loop will run for
        """
        self.epsilon = max(self.epsilon*0.997,0.1)
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        if explore and cond < self.epsilon :
            return np.random.randint(0,self.action_space.n)
        else:
            tensor = torch.Tensor(obs)
            y = self.critics_target.forward(tensor).detach()
            return np.argmax(y.numpy())
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        # Target Q Values
        target = self.critics_net.forward(getattr(batch, "states")).gather(1,getattr(batch, "actions").long()).t()
        
        # Expected Q Values
        max_Q = (torch.max(self.critics_target.forward(getattr(batch, "next_states")),1).values)
        pred = getattr(batch, "rewards").t() + self.gamma*(1 - getattr(batch, "done")).t() * max_Q

        # Setting up MSE Loss
        loss = torch.nn.MSELoss()
        MSE = loss(pred,target)

        # Initializing the gradient to # optimize the model
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        if explore:
            ...
            tensor = torch.Tensor(obs)
            y = self.policy.forward(tensor).detach()
            return np.argmax(y.numpy())
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        G = 0
        p_loss = 0
        for s,a,r in zip(observations,actions,rewards):       
            G = r + self.gamma*G
            tensor = torch.Tensor(s)
            action = self.policy.forward(tensor)
            p_loss += -torch.log(action[a])*G
        
        p_loss = p_loss / len(actions)
